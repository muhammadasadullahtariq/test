...
export default class AddACarStepOne extends React.Component {
    ...
    return {
      ...
      headerTitle: (
        <View style={styles.appMainTitle}>
          <Text style={styles.main_center}>
            <Translation str="build_your_ad" />
          </Text>
          <Text style={styles.center}>
            <Translation str="step_1" />
          </Text>
        </View>
      ),
      headerLeft: (
        <View>
          <TouchableOpacity
            ...
            onPress={() => {
              !_this.props.navigation.state.params.editMode
          ...
          (position) => {
            ...
            if (coord.latitude != 0 && coord.longitude != 0) {
              ...
              getLocationName(
                coord.latitude,
                coord.longitude,
                _this.state.apiKeyAndroid
              ).then(
                (responseJson) => {
                  if (
                    responseJson.hasOwnProperty("results") &&
                    responseJson.results.length > 0
                  ) {
                    addACarData["stm_location_text"] =
  ...
  async componentWillMount() {
    ...
    if (_this.props.navigation.state.params.editMode) {
      await getEditCar(_this.props.navigation.state.params.listingId).then(
        (responceJSON) => {
          addACarData["stm_current_car_id"] =
            _this.props.navigation.state.params.listingId;
              ...
              if (index.length > 0) {
                switch (index) {
                  case "seller_notes":
                    return (
                      <View key={index} style={styles.addMediaWrap}>
                        <Text
                          style={[
                            styles.filterMediaLabel,
                            _this.state.checkValidate &&
                            validateFields.hasOwnProperty(index) &&
                            !validateFields[index]
                              ? styles.error
                              : "",
                          ]}
                        >
                          <Translation str="seller_note" />
                          <Text styles={styles.required}>*</Text>
                        </Text>
                        <View style={styles.textareaWrap}>
                          <TextInput
                            ...
                            defaultValue={
                              addACarData.hasOwnProperty("stm_seller_notes")
                                ? addACarData["stm_seller_notes"]
                                : ""
                            }
                          />
                        </View>
                  ...
                  case "stm_additional_features":
                    if (
                      !addACarData.hasOwnProperty(_this.state.additional_key)
                    ) {
                          ...
                          renderItem={({ item }) => (
                            <TouchableOpacity
                              ...
                              onPress={() => {
                                if (
                                  !addACarData[
                                    _this.state.additional_key
                                  ].hasOwnProperty(item.slug)
                                ) {
                                  addACarData[_this.state.additional_key][
                                    item.slug
                                  ] = item.label;
                  ...
                  case "add_media":
                    return (
                      <View key={index} style={styles.addMediaWrap}>
                        ...
                        >
                          <Translation str="add_media" />
                          <Text styles={styles.required}>*</Text>
                        </Text>

                        {addACarData.hasOwnProperty("images") &&
                        addACarData.images.hasOwnProperty(0) ? (
                              ...
                              onPress={() => {
                                _this.props.navigation.navigate(
                                  "AddACarChooseImageDragabble",
                                  {
                                    ...
                                    _setFilterSelectParams:
                                      _this._setFilterSelectParams,
                                  }
                                );
                              }}
                            >
                              <Image
                                source={{
                                  uri: addACarData.images.hasOwnProperty(0)
                                    ? `data:image/jpg;base64,${addACarData.images[0]["src"]}`
                                    : addACarData.images[0]["src"],
                                }}
                                style={styles.img}
                              />
                      ...
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          _this.props.navigation.navigate("AddACarMakeChoose", {
                            ...
                            _setCheckedFilterParams:
                              _this._setCheckedFilterParams,
                          });
                        }}
                      >
                        <View style={styles.multiPicker}>
                          ...
                          >
                            <Translation str="make" />
                            <Text styles={styles.required}>*</Text>
                          </Text>
                          <View style={styles.checkedItems}>
                            {addACarData.hasOwnProperty("stm_f_s_make") &&
                            Object.keys(addACarData.stm_f_s_make).length > 0 ? (
                              _.map(
                                addACarData["stm_f_s_make"],
                                function (val, slug) {
                                  return (
                                    <TouchableOpacity
                                      activeOpacity={0.8}
                    ...
                    return (
                      ...
                      >
                        <View style={styles.multiPicker}>
                          ...
                          <View style={styles.checkedItems}>
                            {addACarData.hasOwnProperty("stm_f_s_body") &&
                            Object.keys(addACarData.stm_f_s_body).length > 0 ? (
                              _.map(
                                addACarData["stm_f_s_body"],
                                function (val, slug) {
                                  return (
                                    <TouchableOpacity
                                      activeOpacity={0.8}
                      ...
                      <View key={index} style={styles.multiPicker}>
                        ...
                        <View style={styles.inputWrap}>
                          <TextInput
                            ...
                            defaultValue={
                              addACarData.hasOwnProperty("stm_f_s_mileage")
                                ? addACarData["stm_f_s_mileage"]
                                : ""
                            }
                          />
                      ...
                      <View key={index} style={styles.multiPicker}>
                        ...
                        <View style={styles.inputWrap}>
                          <TextInput
                            ...
                            keyboardType="numeric"
                            defaultValue={
                              addACarData.hasOwnProperty("stm_f_s_engine")
                                ? addACarData["stm_f_s_engine"]
                                : ""
                            }
                            onChangeText={(text) => {
                              _this._validateTextFields(index, text);
                      ...
                      <View key={index} style={styles.multiPicker}>
                        ...
                        <View style={styles.inputWrap}>
                          <TextInput
                            ...
                            keyboardtype="numeric"
                            defaultValue={
                              addACarData.hasOwnProperty("stm_car_price")
                                ? addACarData["stm_car_price"]
                                : ""
                            }
                            onChangeText={(text) => {
                              _this._validateTextFields(index, text);
                          ...
                          />
                          <Text style={styles.abs}>
                            ({_this.state.currencyName})
                          </Text>
                          <Text style={styles.abs_left}>
                            {_this.state.currency}
                          </Text>
                        </View>
                      </View>
                    );
                        ...
                        <View style={styles.locationMainWrap}>
                          ...
                          <View style={styles.locationInputWrap}>
                            <GooglePlacesAutocomplete
                              defaultValue={
                                addACarData.hasOwnProperty("stm_location_text")
                                  ? addACarData["stm_location_text"]
                    ...
                    return (
                      ...
                      >
                        <View style={styles.multiPicker}>
                          ...
                          <View style={styles.checkedItems}>
                            {addACarData.hasOwnProperty("stm_f_s_" + index) &&
                            Object.keys(addACarData["stm_f_s_" + index])
                              .length > 0 ? (
                              _.map(
                                addACarData["stm_f_s_" + index],
                                function (val, slug) {
                                  return (
                                    <TouchableOpacity
                                      activeOpacity={0.8}
